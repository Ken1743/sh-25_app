...
/** 13 -> "scene1 option3", 24 -> "scene2 option4" */
function codeToKey(n) {
if (typeof n !== "number" || !Number.isFinite(n)) return null;
const scene = Math.floor(n / 10);
const option = n % 10;
if (scene <= 0 || option <= 0) return null; // 0は無視（必要なら仕様変更OK）
return `scene${scene} option${option}`;
}

/** 入力を events配列に正規化
 * - 既存形式: [{key:"scene1 option1"}, ...]
 * - 新形式  : { clickedObjects: [13,12,...] }
 */
...
if (Array.isArray(input)) {
    ...
    .map(e => ({ key: e.key.replace(/\s+/g, " ") }));
}
// 新形式（オブジェクト）: clickedObjects から変換
if (input && Array.isArray(input.clickedObjects)) {
    const out = [];
    for (const n of input.clickedObjects) {
    const key = codeToKey(n);
    if (key) out.push({ key });
    }
    return out;
}
throw new Error("events must be an array of {key} or an object with {clickedObjects:number[]}");
}
...
// events[] を時間付きに解決（duration/start/metaは base を採用）
...
// === 外部公開：calc(events) ※設定は内部ロード。eventsは旧/新どちらの形式でも可
function calc(eventsInput) {
const model = loadModel();

// ★ 入力を正規化（clickedObjects -> events[] / 既存配列はそのまま）
